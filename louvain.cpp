...
#pragma omp parallel default(shared), shared(localCinfo, localCupdate)
    {
...
GraphWeight distLouvainMethod(const int me, const int nprocs, const DistGraph &dg,
        ...
        std::vector<GraphElem> &svdata, std::vector<GraphElem> &rvdata,
        CommunityVector &cvect, const GraphWeight lower, const GraphWeight thresh, 
        int& iters, GpuGraph &gpu_graph)
{
  const GraphElem numGhosts = dg.getNumGhosts(me);
  CommunityVector pastComm, currComm, targetComm;
  GraphWeightVector vDegree;
  GraphWeightVector clusterWeight;
  CommVector localCinfo, localCupdate;
...
#endif
  MPI_Barrier(MPI_COMM_WORLD);
  // if( me == 0 ) std::cout << "=== ITERATION: " << numIters << std::endl;
    distCleanCWandCU(nv, clusterWeight, localCupdate);
    assert( gpu_for_louvain_iteration(
                    nv, dg, currComm, targetComm, vDegree, localCinfo,
                    localCupdate, remoteComm, remoteCinfo, remoteCupdate,
                    constantForSecondTerm, clusterWeight, me, numIters, gpu_graph));
// if(me == 0) std::cout << "nv[" << nv << "]; Line[" 
//      << __LINE__ << "]" << std::endl;
#ifdef DEBUG_PRINTF  
    t1 = MPI_Wtime();
    ofs << "Iteration computation time: " << (t1 - t0) << std::endl;
...
//      << __LINE__ << "]" << std::endl;
    ...
    currMod = distComputeModularity(g, localCinfo, clusterWeight, constantForSecondTerm, 
              me, gpu_graph);
// if(me == 0) std::cout << "nv[" << nv << "]; Line[" 
//      << __LINE__ << "]" << std::endl;
...
#pragma omp parallel for shared(clusterWeight, localCupdate, currComm, targetComm, \
        vDegree, localCinfo, remoteCinfo, remoteComm, dg, remoteCupdate, pastComm), \
          ...
          // update local cinfo
#ifdef OMP_SCHEDULE_RUNTIME
#pragma omp parallel for default(shared), shared(localCinfo, localCupdate) schedule(runtime)
#else
#pragma omp parallel for default(shared), shared(localCinfo, localCupdate) schedule(static)
#endif          
          for (GraphElem i = 0; i < nv; i++) {
...
GraphWeight distComputeModularity(const Graph &g, CommVector &localCinfo,
			     const GraphWeightVector &clusterWeight,
			     const GraphWeight constantForSecondTerm,
			     const int me, GpuGraph &gpu_graph)
{
  const GraphElem nv = g.getNumVertices();
...
#ifdef RUNONGPU
// #if 0
  gpu_reduce_mod(localCinfo, clusterWeight, gpu_graph, le_la_xx, nv);
#else
...
#pragma omp parallel for shared(rcsizes) \
  ...
  for (int i = 0; i < nprocs; i++) {
      if (i != me) {
#ifdef OMP_SCHEDULE_RUNTIME
#pragma omp parallel for default(shared), shared(rcsizes, rcomms, localCinfo, sinfo, rdispls), \
...
#ifdef USE_OPENMP_LOCK
#pragma omp parallel default(shared), shared(dg, g, locks, parray)
#else
#pragma omp parallel default(shared), shared(dg, g, parray)
#endif
  {
#ifdef OMP_SCHEDULE_RUNTIME
#pragma omp for schedule(runtime)
